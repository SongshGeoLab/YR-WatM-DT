...
#### **Page 2: Water Availability - 水资源可用性**
完整的气候变化情景分析页面，展示多情景聚合与不确定性可视化。

##### 核心功能
1. **气候情景选择**
   - RCP2.6 (低排放)、RCP4.5 (中排放)、RCP8.5 (高排放)
   - 与全局参数联动
   - 实时更新所有图表
...
3. **智能情景显示**
   - **单情景模式**：当参数筛选出唯一情景时，显示单条曲线
   - **多情景模式**：当匹配多个情景时，显示均值 + 不确定性区间 (min/max)
   - 自动检测并显示情景数量
   - 实时切换，无需手动配置
...
│   │   ├── DataComparisonPanel.tsx           # **NEW**: 气候影响对比面板
...
│   │   └── useClimateComparison.ts           # **NEW**: 气候数据对比
│   ├── services/
│   │   └── api.ts                            # API 客户端
│   └── styles/
│       └── globals.css                       # 全局样式
